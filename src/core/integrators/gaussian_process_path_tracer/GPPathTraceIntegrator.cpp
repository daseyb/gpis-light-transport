#include "GPPathTraceIntegrator.hpp"
...
CONSTEXPR uint32 GPPathTraceIntegrator::TileSize;
CONSTEXPR uint32 GPPathTraceIntegrator::VarianceTileSize;
CONSTEXPR uint32 GPPathTraceIntegrator::AdaptiveThreshold;

GPPathTraceIntegrator::GPPathTraceIntegrator()
: Integrator(),
  _w(0),
  _h(0),
  _varianceW(0),
...
void GPPathTraceIntegrator::diceTiles()
{
...
float GPPathTraceIntegrator::errorPercentile95()
{
...
void GPPathTraceIntegrator::dilateAdaptiveWeights()
{
...
void GPPathTraceIntegrator::distributeAdaptiveSamples(int spp)
{
...
bool GPPathTraceIntegrator::generateWork()
{
    for (SampleRecord &record : _samples)
...
void GPPathTraceIntegrator::renderTile(uint32 id, uint32 tileId)
{
...
void GPPathTraceIntegrator::saveState(OutputStreamHandle &out)
{
...
void GPPathTraceIntegrator::loadState(InputStreamHandle &in)
{
    for (SampleRecord &s : _samples)
        s.loadState(in);
    for (ImageTile &i : _tiles)
...
void GPPathTraceIntegrator::fromJson(JsonPtr value, const Scene &/*scene*/)
{
    _settings.fromJson(value);
}

rapidjson::Value GPPathTraceIntegrator::toJson(Allocator &allocator) const
{
    return _settings.toJson(allocator);
}

void GPPathTraceIntegrator::prepareForRender(TraceableScene &scene, uint32 seed)
{
    _currentSpp = 0;
    ...
    for (uint32 i = 0; i < ThreadUtils::pool->threadCount(); ++i)
        _tracers.emplace_back(new GPPathTracer(&scene, _settings, i));
...
void GPPathTraceIntegrator::teardownAfterRender()
{
    _group.reset();
...
bool GPPathTraceIntegrator::supportsResumeRender() const
{
    return true;
}

void GPPathTraceIntegrator::startRender(std::function<void()> completionCallback)
{
    ...
    _group = ThreadUtils::pool->enqueue(
        std::bind(&GPPathTraceIntegrator::renderTile, this, _3, _1),
        _tiles.size(),
        [&, completionCallback]() {
            _currentSpp = _nextSpp;
            advanceSpp();
...
void GPPathTraceIntegrator::waitForCompletion()
{
    if (_group) {
        _group->wait();
        _group.reset();
...
void GPPathTraceIntegrator::abortRender()
{
    if (_group) {
        _group->abort();
        _group->wait();
